...
describe.skipIf(isWindows)('CTRL+C Library Tests (command-stream)', () => {
  ...
  it('should handle command cancellation with kill()', async () => {
    ...
    try {
      ...
      trace(
        'LibraryTest',
        () => `Command interrupted with error: ${error.message}`
      );
  ...
  it.skip('should test our library via external script - SKIP: uses test-sleep.mjs', async () => {
    ...
    child.stderr.on('data', (data) => {
      ...
      trace(
        'LibraryTest',
        () =>
          `Received stderr (first 100): ${data.toString().substring(0, 100)}`
      );
    });
    ...
    let attempts = 0;
    while (!dataReceived && attempts < 30) {
      // 3 seconds total
    ...
    trace(
      'LibraryTest',
      () => `Final stderr (first 300): ${stderr.substring(0, 300)}`
    );
  ...
  it('should handle virtual command cancellation', async () => {
    ...
    try {
      ...
      trace(
        'LibraryTest',
        () => `Virtual command result (normal completion): ${result.code}`
      );
