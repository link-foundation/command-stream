...
echoTest
  ...
  .on('end', (result) => {
    console.log(`✅ Echo test complete. Chunks: ${chunkCount}, Exit code: ${result.code}\n`);
...
function runSeqTest() {
  ...
  seqTest
    ...
    .on('end', (result) => {
      console.log(`✅ Seq test complete. Chunks: ${seqChunks}, Exit code: ${result.code}\n`);
...
async function runAsyncIteratorTest() {
  ...
  for await (const chunk of $`seq 10 13`.stream()) {
    iteratorChunks++;
    console.log(`🔄 Iterator chunk ${iteratorChunks}: "${chunk.data.toString().trim()}" (${chunk.type})`);
  }
