...
export function redact(value, placeholder) {
  if (value) redactions.push([value, placeholder]);
}
...
function sanitize(value) {
  if (typeof value === 'string') {
    ...
    for (const [needle, placeholder] of [...redactions].sort((a, b) => b[0].length - a[0].length)) {
...
export async function example(meta, body) {
  ...
  if (process.env.COMMAND_STREAM_PARITY === '1') {
    ...
    console.log(JSON.stringify({ id: meta.id, runtime, observations, failure }));
