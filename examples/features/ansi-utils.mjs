...
// captured output.
import { AnsiUtils, processOutput, configureAnsi, getAnsiConfig } from '../../src/$.mjs';
import { example } from './_harness.mjs';

const ESC = String.fromCharCode(27);
const BELL = String.fromCharCode(7);

await example({ id: 'ansi-utils', title: 'ANSI and control character helpers' }, async ({ record }) => {
  ...
  record('stripControlChars keeps text readable', AnsiUtils.stripControlChars(`beep${BELL}boop`));
  record('stripAll does both', AnsiUtils.stripAll(`${ESC}[31mred${ESC}[0m${BELL}`));
  record('cleanForProcessing handles buffers', AnsiUtils.cleanForProcessing(Buffer.from(coloured)).toString());
