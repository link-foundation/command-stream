...
await example({ id: 'events', title: 'EventEmitter interface' }, async ({ record }) => {
  const events = [];
  ...
  record('events (sorted: stdout/stderr order is up to the OS)', events.sort());

  // The 'data' event receives both streams with a type tag.
  const tagged = [];
  await new Promise(resolve => {
    $q`echo tagged`
      .on('data', chunk => tagged.push([chunk.type, chunk.data.toString().trim()]))
      .on('end', () => resolve())
      .start();
  });
