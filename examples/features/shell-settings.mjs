...
await example({ id: 'shell-settings', title: 'Shell settings' }, async ({ record }) => {
  ...
  shell.pipefail(true);
  record('pipefail makes an early failure win', (await $q`sh -c 'exit 3' | cat`).code);
  shell.pipefail(false);
  record('without pipefail the last stage wins', (await $q`sh -c 'exit 3' | cat`).code);
