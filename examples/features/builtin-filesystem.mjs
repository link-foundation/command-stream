...
await example({ id: 'builtin-filesystem', title: 'File system built-ins' }, async ({ record }) => {
  const dir = makeTempDir('fs');
  const $q = $({ mirror: false, cwd: dir });

  await $q`mkdir -p project/src`;
  record('mkdir -p created the tree', fs.existsSync(path.join(dir, 'project/src')));

  await $q`touch project/src/index.mjs`;
  record('touch created the file', fs.existsSync(path.join(dir, 'project/src/index.mjs')));
  ...
  record('the tree still exists after rm -rf', fs.existsSync(path.join(dir, 'project')));
});
