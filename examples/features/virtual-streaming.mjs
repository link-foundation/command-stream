...
await example({ id: 'virtual-streaming', title: 'Streaming commands' }, async ({ record }) => {
  ...
  record('same command awaited as a whole', (await $({ mirror: false })`countdown 2`).stdout);

  // Streaming commands compose with the rest of a pipeline.
  record('piped into a built-in', (await $({ mirror: false })`countdown 2 | cat`).stdout);
