...
async function testSigintForwarding() {
  ...
  console.log('\n1. Initial SIGINT listeners:', process.listeners('SIGINT').length);
  
  console.log('\n2. Starting a long-running command...');
  const runner = $`sleep 5`;
  const promise = runner.start();
  
  console.log('SIGINT listeners after starting:', process.listeners('SIGINT').length);
  
  // Send SIGINT after a short delay
  setTimeout(() => {
    console.log('\n3. Sending SIGINT to parent process...');
    console.log('Active listeners before SIGINT:', process.listeners('SIGINT').length);
