...
async function testNativeSpawnVsCommandStream() {
  try {
    console.log('TEST 1: Native spawn - try to kill ping via stdin (will fail)');
    ...
    console.log('  Native ping terminated with code:', nativeResult.code, 'signal:', nativeResult.signal);
    console.log('  Output length:', nativeResult.stdout.length);
    
    console.log('');
    console.log('TEST 2: command-stream - kill ping using kill() method (will work)');
    ...
    console.log('  command-stream ping terminated with code:', streamResult.code);
    console.log('  Output length:', streamResult.stdout.length);
    
    console.log('');
    console.log('TEST 3: Show stdin works for commands that actually read stdin');
    ...
    console.log('  command-stream cat result:', JSON.stringify(streamCatResult.stdout));
    
    console.log('');
    console.log('✅ CONCLUSIONS:');
    console.log('  1. Native spawn cannot kill ping via stdin CTRL+C (ping ignores stdin)');
    console.log('  2. command-stream kill() method properly terminates ping with signals');
    console.log('  3. Both native spawn and command-stream work fine with stdin for commands that read it (like cat)');
    console.log('  4. For ping specifically, you need kill() method, not stdin manipulation');
