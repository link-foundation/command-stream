...
function getInternalState() {
  ...
  return {
    ...
    activeProcessRunners: activeProcessRunners
  };
}
...
async function testSigintHandlerInstall() {
  ...
  console.log('After creating runner1, SIGINT listeners:', getSigintListenerCount());
  
  console.log('\n3. Starting first command...');
  const promise1 = runner1.start();
  console.log('After starting runner1, SIGINT listeners:', getSigintListenerCount());
  
  console.log('\n4. Creating second command runner while first is running...');
  const runner2 = $`sleep 0.01`;
  const promise2 = runner2.start();
  console.log('After starting runner2, SIGINT listeners:', getSigintListenerCount());
  ...
  await runner3;
  console.log('After third command finished, SIGINT listeners:', getSigintListenerCount());
}
