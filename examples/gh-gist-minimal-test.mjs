...
try {
  ...
  const result = await $`timeout 10 gh gist create ${testFile} --desc "minimal-test" --public=false`.run({
    capture: true,
...
try {
  ...
  const result = await $`gh gist create ${testFile} --desc "minimal-test-2" --public=false`.run({
    capture: true,
    mirror: true,
