...
try {
  ...
  const result = await $`gh gist create ${testFile} --desc "hang-test" --public=false`.run({
    capture: true,
...
try {
  ...
  const result = await $`gh gist create ${testFile} --desc "hang-test-2" --public=false 2>&1`.run({
    capture: true,
    mirror: false,
    timeout: 5000
  });
  ...
  console.log('Stdout length:', result.stdout?.length);
  console.log('Stderr length:', result.stderr?.length);
  console.log('Stdout content (first 200 chars):', result.stdout?.slice(0, 200));
  console.log('Stderr content:', result.stderr?.trim());
...
try {
  ...
  const result = await $`gh gist create ${testFile} --desc "hang-test-3" --public=false`.run({
    capture: true,
...
console.log('- Issue: gh outputs progress to stderr, and $.mjs might not handle it well');
console.log('- Workaround 1: Use 2>&1 to redirect stderr to stdout');
console.log('- Workaround 2: Use mirror: true to display output while capturing');