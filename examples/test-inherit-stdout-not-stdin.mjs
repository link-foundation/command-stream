...
async function testInheritStdoutNotStdin() {
  try {
    ...
    const result1 = await inheritStdoutCmd.run({ stdout: 'inherit', stdin: 'pipe' });
    console.log('✓ Command completed with inherit stdout');
    console.log('  Exit code:', result1.code);
    console.log('  Captured stdout (should be empty due to inherit):', JSON.stringify(result1.stdout));
    ...
    console.log('  3b: Inherit stdout, ignore stdin (for commands that don\'t need stdin)');
    const result3b = await $`echo "Direct output"`.run({ stdout: 'inherit', stdin: 'ignore' });
    ...
    console.log('');
    console.log('TEST 4: Verify top command with inherited stdout and controlled stdin');
    
    console.log('  → Starting top with inherited stdout, sending "q" via stdin...');
    ...
    console.log('  • This combination allows seeing output while controlling input');
    console.log('  • Perfect for interactive commands that need specific input');
