...
async function proveStdinLimitation() {
  ...
  console.log(`  ✓ command-stream ping cleanly terminated with code: ${result.code}`);
  console.log('');
  ...
  console.log('📋 SUMMARY:');
  console.log('  • ping ignores stdin completely (network utility, not interactive)');
  console.log('  • To interrupt ping, you MUST use kill() method or signals');
  console.log('  • stdin works fine for commands that actually read it (cat, grep, etc.)');
  console.log('  • command-stream provides both: streams.stdin AND kill() method');
}
