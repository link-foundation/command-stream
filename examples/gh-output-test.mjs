...
try {
  ...
  console.log('Stdout content:', result.stdout ? `"${result.stdout.slice(0, 50)}..."` : '(empty)');
  console.log('Stdout length:', result.stdout?.length || 0);
  console.log('Stderr content:', result.stderr ? `"${result.stderr.slice(0, 50)}..."` : '(empty)');
...
try {
  ...
  console.log('Exit code:', result.code);
  console.log('Combined stdout content:', result.stdout ? `"${result.stdout.slice(0, 50)}..."` : '(empty)');
  console.log('Combined stdout length:', result.stdout?.length || 0);
  console.log('Stderr after redirect:', result.stderr ? `"${result.stderr.slice(0, 50)}..."` : '(empty)');
  console.log('Stderr length:', result.stderr?.length || 0);
...
try {
  ...
  const result2 = await $`gh auth status 2>&1`.run({ capture: true, mirror: false });
