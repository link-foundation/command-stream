...
try {
  ...
  const result = await $`gh gist create ${tempFile} --desc "original-hang-test" --secret`.run({ 
    capture: true, 
    mirror: false
...
try {
  const result = await $`gh gist create ${tempFile} --desc "public-false-test" --public=false`.run({ 
    capture: true, 
    mirror: false
  });
