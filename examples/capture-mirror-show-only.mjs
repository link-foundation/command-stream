...
console.log('await $`echo "Shown but not captured"`.start({ capture: false, mirror: true })');
const result = await $`echo "Shown but not captured"`.start({ capture: false, mirror: true });
