...
async function testInteractiveStreaming() {
  ...
  const calcPath = new URL('./interactive-math-calc.mjs', import.meta.url).pathname;
  ...
  stdout.on('data', (chunk) => {
    ...
    if (text.includes('RESULT:') || text.includes('READY') || text.includes('GOODBYE')) {
  ...
  console.log('Total results received:', results.filter(r => r.includes('RESULT:')).length);
  console.log('\nAll results:');
  results.forEach(r => {
