...
for await (const chunk of proc2.stdout) {
  ...
  if (chunkCount === 1) {
    console.log(`[${elapsed}ms] First chunk arrived with ${lines.length} lines`);
    console.log('First few lines:', lines.slice(0, 3));
  } else {
    console.log(`[${elapsed}ms] Chunk ${chunkCount} with ${lines.length} lines`);
  }
}

console.log(`\nTotal chunks received: ${chunkCount}`);
console.log('If streaming worked, we should see multiple chunks at different times');
