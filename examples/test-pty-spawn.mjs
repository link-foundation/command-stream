...
console.log('  Bun.spawn options:', Object.keys(Bun.spawn(['echo'], { stdout: 'pipe' })));
...
console.log('\nUsing script command as PTY wrapper:');
{
  ...
  const proc = Bun.spawn(['script', '-q', '/dev/null', 'sh', '-c', 'bun run examples/emulate-claude-stream.mjs | jq .'], {
    stdout: 'pipe',
    stderr: 'pipe',
    env: {
      ...process.env,
