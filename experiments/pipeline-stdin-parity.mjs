...
console.log(
  `[${runtime}] echo abc | upper          ->`,
  JSON.stringify((await $q`echo abc | upper`).stdout)
);
console.log(
  `[${runtime}] echo abc | show-stdin     ->`,
  JSON.stringify((await $q`echo abc | show-stdin`).stdout)
);
console.log(
  `[${runtime}] seq 1 3 | show-stdin      ->`,
  JSON.stringify((await $q`seq 1 3 | show-stdin`).stdout)
);
console.log(
  `[${runtime}] sh -c echo | show-stdin   ->`,
  JSON.stringify((await $q`sh -c 'echo sys' | show-stdin`).stdout)
);
console.log(
  `[${runtime}] upper (no pipe)           ->`,
  JSON.stringify((await $q`upper`).stdout)
);
