...
const result2 = await $`echo "${testText}"`.run({ capture: true, mirror: false });
...
const result4 = await $`sh -c 'echo "didn'"'"'t"'`.run({ capture: true, mirror: false });
...
console.log(`   The quote() function produces: '${testText.replace(/'/g, "'\\''")}'`);
console.log(`   This should expand to: ${testText} when interpreted by the shell`);
...
const result6 = await $`sh -c ${shellCmd}`.run({ capture: true, mirror: false });
...
console.log(`   Note: quote("${testText}") returns: '${testText.replace(/'/g, "'\\''")}' (single-quoted with escaped apostrophe)`);
process.env.COMMAND_STREAM_VERBOSE = verbose;

// Test 8: Direct execution of expected result
console.log('\n8. Direct execution with raw:');
const result8 = await $`${raw(`echo '${testText}'`)}`.run({ capture: true, mirror: false });
