...
const probe = async (label, fn) => {
  try {
    const r = await fn();
    console.log(
      `[${runtime}] ${label.padEnd(34)} -> code=${r.code} stdout=${JSON.stringify(r.stdout)}`
    );
  } catch (e) {
    console.log(
      `[${runtime}] ${label.padEnd(34)} -> THREW ${JSON.stringify(e.message)} code=${e.code}`
    );
  }
};
...
const real =
  await $q`sh -c 'set -o pipefail; sh -c "exit 3" | cat; echo code=$?'`;
console.log(
  `[${runtime}] real shell with pipefail        -> ${JSON.stringify(real.stdout)}`
);
