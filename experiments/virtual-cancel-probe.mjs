...
register('cancellable', async ({ abortSignal, isCancelled }) => {
  events.push(['handler start', { hasSignal: !!abortSignal, aborted: abortSignal?.aborted }]);
  abortSignal?.addEventListener?.('abort', () => events.push(['abort event', true]));
  for (let i = 0; i < 20; i++) {
    if (abortSignal?.aborted) { events.push(['saw aborted at', i]); break; }
    if (isCancelled?.()) { events.push(['saw isCancelled at', i]); break; }
...
runner.start();
setTimeout(() => { events.push(['kill called', null]); runner.kill(); }, 50);
