...
async function probe(label, run, file) {
  const r = await run();
  console.log(
    `[${runtime}] ${label.padEnd(30)} code=${r.code} stdout=${JSON.stringify(r.stdout)} stderr=${JSON.stringify(r.stderr.trim())} file=${fs.existsSync(file) ? JSON.stringify(fs.readFileSync(file, 'utf8')) : 'MISSING'}`
  );
}
...
console.log(
  `[${runtime}] ${'cat < f'.padEnd(30)} code=${r.code} stdout=${JSON.stringify(r.stdout)} stderr=${JSON.stringify(r.stderr.trim())}`
);
