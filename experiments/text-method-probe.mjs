...
const report = (label, value) => console.log(`${label.padEnd(34)} text(): ${typeof value.text}`);
...
report('system command (sync)', $({ mirror: false })`sh -c 'echo system'`.sync());
report('pipeline (async)', await $q`echo a | cat`);
report('.pipe() method', await $({ mirror: false })`echo a`.pipe($({ mirror: false })`cat`));
