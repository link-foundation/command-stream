...
out(
  'pipe-method',
  (await $({ mirror: false })`echo pm`.pipe($({ mirror: false })`upper`)).stdout
);
unregister('upper');

register('gen', async function* ({ args }) {
  for (let i = 1; i <= Number(args[0] || 2); i++) {
    yield `n${i}\n`;
  }
});
...
out(
  'opts-stdin',
  (await $({ mirror: false, stdin: 'from-stdin\n' })`cat`).stdout
);
...
out(
  'exec-fn',
  (await exec('echo', ['execfn'], { mirror: false, capture: true })).stdout
);
