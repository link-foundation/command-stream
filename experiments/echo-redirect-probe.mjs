...
console.log(
  `[${runtime}] write code=${w.code} stdout=${JSON.stringify(w.stdout)} stderr=${JSON.stringify(w.stderr)}`
);
const r = await $q`cat ${f}`;
console.log(
  `[${runtime}] read  code=${r.code} stdout=${JSON.stringify(r.stdout)} stderr=${JSON.stringify(r.stderr)}`
);
await $q`rm -rf ${dir}`;
