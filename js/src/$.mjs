...
// Import from extracted utility modules
...
import { quote, buildShellCommand, pumpReadable, raw } from './$.quote.mjs';
...
// Create accessors for state variables to maintain compatibility
// These are getters that return current values from the state module
const globalShellSettings = new Proxy(
  {},
  {
    get(_, prop) {
      return getShellSettings()[prop];
    },
    set(_, prop, value) {
      const current = getShellSettings();
      current[prop] = value;
      setShellSettings(current);
      return true;
    },
    ownKeys() {
      return Object.keys(getShellSettings());
    },
    getOwnPropertyDescriptor(_, prop) {
      const val = getShellSettings()[prop];
      if (val !== undefined) {
...
class ProcessRunner extends StreamEmitter {
  ...
  start(options = {}) {
    ...
    if (Object.keys(options).length > 0 && !this.started) {
      ...
      if (
        ...
        this.options.signal.addEventListener('abort', () => {
          ...
          trace(
            'ProcessRunner',
            () =>
              `External abort signal received - killing process | ${JSON.stringify(
                {
                  hasChild: !!this.child,
                  childPid: this.child?.pid,
                  finished: this.finished,
          ...
          if (this._abortController && !this._abortController.signal.aborted) {
            ...
            trace(
              'ProcessRunner',
              () =>
                `Cannot abort internal controller | ${JSON.stringify(
                  {
                    hasInternalController: !!this._abortController,
                    internalAlreadyAborted:
        ...
        if (this.options.signal.aborted) {
          trace(
            'ProcessRunner',
            () =>
              `External signal already aborted, killing process and aborting internal controller | ${JSON.stringify(
                {
                  hasInternalController: !!this._abortController,
                  internalAborted: this._abortController?.signal.aborted,
                },
          ...
          trace(
            'ProcessRunner',
            () =>
              `Signal already aborted - killing process immediately | ${JSON.stringify(
                {
                  hasChild: !!this.child,
                  childPid: this.child?.pid,
                  finished: this.finished,
          ...
          trace(
            'ProcessRunner',
            () => 'Process kill initiated due to pre-aborted signal'
          );
          ...
          if (this._abortController && !this._abortController.signal.aborted) {
            ...
            trace(
              'ProcessRunner',
              () =>
                `Internal controller aborted immediately | ${JSON.stringify(
                  {
                    internalAborted: this._abortController?.signal?.aborted,
                  },
                  null,
        ...
        trace(
          'ProcessRunner',
          () =>
            `No external signal to handle | ${JSON.stringify(
              {
                hasSignal: !!this.options.signal,
      ...
      trace(
        'ProcessRunner',
        () =>
          `BRANCH: options => IGNORED_ALREADY_STARTED | ${JSON.stringify({}, null, 2)}`
      );
    }
    ...
    try {
          ...
          if (parsed.type === 'pipeline') {
            ...
            if (shouldBypassVirtual) {
              ...
              trace(
                'ProcessRunner',
                () =>
                  `Executing virtual command | ${JSON.stringify(
                    {
                      cmd: parsed.cmd,
      ...
      const spawnBun = (argv) => {
        ...
        if (isInteractive) {
          ...
          trace(
            'ProcessRunner',
            () =>
              `spawnBun: Interactive process created | ${JSON.stringify(
                {
                  pid: child.pid,
                  killed: child.killed,
                },
        ...
        trace(
          'ProcessRunner',
          () =>
            `spawnBun: About to spawn | ${JSON.stringify(
              {
                argv,
                cwd,
                shellCmd: argv[0],
        ...
        trace(
          'ProcessRunner',
          () =>
            `spawnBun: Non-interactive process created | ${JSON.stringify(
              {
                pid: child.pid,
                killed: child.killed,
      ...
      if (this.child) {
        ...
        if (this.child && typeof this.child.on === 'function') {
          ...
          trace(
            'ProcessRunner',
            () =>
              `Skipping event listeners - child does not support .on() method (likely Bun process)`
          );
        }
      } else {
        trace(
          'ProcessRunner',
          () =>
            `No child process created | ${JSON.stringify(
              {
                spec: this.spec,
      ...
      if (stdin === 'inherit') {
        ...
        if (this.child.stdin && typeof this.child.stdin.end === 'function') {
          ...
          trace(
            'ProcessRunner',
            () => `stdin: Child stdin closed successfully`
          );
        }
      } else if (stdin === 'pipe') {
        trace(
        ...
        trace(
          'ProcessRunner',
          () => `stdin: Unhandled stdin type: ${typeof stdin}`
        );
      }
      ...
      trace(
        'ProcessRunner',
        () =>
          `Processing exit code | ${JSON.stringify(
            {
              rawCode: code,
              cancelled: this._cancelled,
              childKilled: this.child?.killed,
      ...
      trace(
        'ProcessRunner',
        () =>
          `About to finish process with result | ${JSON.stringify(
            {
      ...
      trace(
        'ProcessRunner',
        () =>
          `Returning result successfully | ${JSON.stringify(
            {
              exitCode: this.result.code,
              errexit: globalShellSettings.errexit,
            },
      ...
      trace(
        'ProcessRunner',
        () => `_doStartAsync caught error: ${error.message}`
      );
  ...
  _startSync() {
    ...
    trace(
      'ProcessRunner',
      () =>
        `Starting sync execution | ${JSON.stringify({ mode: this._mode }, null, 2)}`
    );
