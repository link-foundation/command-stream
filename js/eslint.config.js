// ESLint runs with `js/` as the working directory (`bun run lint`), while
// lint-staged and editors may invoke it from the repository root. Flat-config
// globs resolve against the working directory, so every override below is
// anchored with `**/` and matches under either one. Anchoring the globs to one
// directory is how `js/tests/**` and `tests/**` ended up duplicated here, with
// one of the two always dead.
...
export default [
  ...
  },
  {
    ignores: [
      ...
      '**/coverage/**',
      '**/dist/**',
      // Generated by `bun run check:duplication`; running it before `bun run
      // lint` used to make the lint step fail on jscpd's bundled prism.js.
      '**/reports/**',
      '**/*.min.js',
      '**/.eslintcache',
      // Build output: rustdoc ships minified JavaScript that is not ours.
      '**/rust/target/**',
      '**/docs/case-studies/**/data/**',
      '**/docs/case-studies/**/log-excerpts/**',
      '**/docs/case-studies/**/templates/**',
      // Collected CI evidence, kept verbatim.
      '**/dev/log/**',
    ],
  },
];
