 ...
 *
 * npm trusted publishing (the keyless, secret-free flow this repo relies on)
 * requires npm >= 11.5.1. The Node.js runtimes used by GitHub Actions ship with
 * older npm, so we must update before publishing. If npm cannot be brought to a
 * version that supports OIDC, publishing would silently fall back to (missing)
 * token auth and fail with an opaque E404 on PUT — one of the failure modes
 * behind issue #166. So this script asserts the final npm version and fails
 * loudly when OIDC is unsupported, instead of letting the publish step discover
 * it later.
 *
 * Ported from the js pipeline template
 * (link-foundation/js-ai-driven-development-pipeline-template,
 * scripts/setup-npm.mjs), adapted to bun. The pure version helpers are exported
 * so tests can exercise the gating logic without fetching dependencies or
 * mutating the global npm install.
 *
...
/**
 * Whether version >= minimumVersion.
 * @param {string} version
...
/**
 * Whether the npm version supports OIDC trusted publishing.
 * @param {string} version
 * @returns {boolean}
 */
...
/**
 * Bring npm to an OIDC-capable version, trying multiple strategies, and assert
 * the result. command-stream's `$` is passed in so this stays testable.
 * @param {Function} $ - command-stream tagged template
 * @param {Function} [fetchFn] - fetch implementation (overridable in tests)
 */
export async function setupNpm($, fetchFn = fetch) {
  ...
  for (const [name, fn] of strategies) {
    ...
    console.warn(
      'This may be a broken bundled-npm runner image (actions/runner-images#13883).'
    );
  }
